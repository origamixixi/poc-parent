...
import org.hc.jds.entity.Cases;
...
import org.hc.jds.repository.db3.CasesTestRepository;
...
public class CasesTest {
    ...
    @Autowired
    private org.hc.jds.repository.db1.Cases200000Repository db1Cases200000Repository;

    @Autowired
    private org.hc.jds.repository.db1.Cases2000000Repository db1Cases2000000Repository;
    ...
    @Test
    public void findAll() {
        List<org.hc.jds.entity.db3.CasesTest> casesTests = casesTestRepository.findAll();
        log.info("casesTest: {}", JSON.toJSONString(casesTests));
    }

    @Test
    public void insertCases200000() {
        insertCases(3_000, 200_000,
                Cases200000.class, cases200000Repository::saveAll,
                org.hc.jds.entity.db1.Cases200000.class, db1Cases200000Repository::saveAll);
    }

    @Test
    public void insertCases2000000() {
        insertCases(3_000, 2_000_000,
                Cases2000000.class, cases2000000Repository::saveAll,
                org.hc.jds.entity.db1.Cases2000000.class, db1Cases2000000Repository::saveAll);
    }

    private <T,V extends Cases> void insertCases(int aggregatedSize, int totalCases,
        ...
        for (int i = 0; i < totalCases; i++) {
            ...
            CompletableFuture<Void> cf1 = CompletableFuture.runAsync(() -> {
                T cases = (T) createCase(prefix, counter, clazz);
                casess.add(cases);
            ...
            CompletableFuture<Void> cf2 = CompletableFuture.runAsync(() -> {
                T cases = (T) createCase(prefix, counter, clazz2);
                casess2.add(cases);

                // Insert batch
                if (casess2.size() >= batchSize) {
                    // Shuffle the list to randomize the order within the batch
                    Collections.shuffle(casess2);

                    // Save all cases in the current batch to the database
                    saveFunction.accept(casess2);
    ...
    private Cases createCase(String prefix, int counter, Class clazz) {
        ...
        return (Cases) JSON.parseObject(JSON.toJSONString(cases), clazz);
    }
