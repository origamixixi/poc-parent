...
public interface Appointment50000003000Repository extends JpaRepository<Appointment50000003000, Integer> {
    ...
    @Query("SELECT a FROM Appointment50000003000 a JOIN Patient20000003000 p on CAST(a.patientNo AS string) = p.patientKey WHERE CAST(a.patientNo AS string) LIKE :hkidPrefix order by a.patientNo")
    ...
    @Query("SELECT a FROM Appointment50000003000 a LEFT JOIN Patient20000003000 p on CAST(a.patientNo AS string) = p.patientKey WHERE CAST(a.patientNo AS string) LIKE :hkidPrefix order by a.patientNo")
    List<Appointment50000003000> typeOfLeftJoinQuery(@Param("hkidPrefix") String hkidPrefix, Pageable pageable);

    @Query("SELECT a FROM Appointment50000003000 a where CAST(a.patientNo AS string) in (select patientKey from Patient20000003000 where patientKey LIKE :hkidPrefix order by patientKey) order by a.patientNo")
    ...
    @Query("""
            ...
            WHERE CAST(a.patientNo AS string) LIKE :hkidPrefix
            order by a.patientNo
            """)
