...
CREATE TABLE public.patient_test (
     ...
     CONSTRAINT patient_exact_dob_fl_ct CHECK (((exact_dob_flag)::text = ANY ((ARRAY['Y'::character varying, 'N'::character varying])::text[]))),
     CONSTRAINT patient_hkid_ct CHECK (((hkid)::text ~* '^[A-Z][A-Z]?[0-9]{6}[0-9A9]{1}$'::text)),
     CONSTRAINT patient_marital_st_ct CHECK (((marital_status)::text = ANY ((ARRAY['M'::character varying, 'D'::character varying, 'S'::character varying, 'W'::character varying, 'U'::character varying])::text[]))),
     ...
     CONSTRAINT patient_sex_ct CHECK (((sex)::text = ANY ((ARRAY['F'::character varying, 'M'::character varying, 'U'::character varying])::text[])))
);
...
CREATE TABLE public.patient_2000000 (
    patient_key varchar(16) NOT NULL,
    religion varchar(6) NULL,
    hkid varchar(24) NOT NULL,
    patient_name varchar(96) NOT NULL,
